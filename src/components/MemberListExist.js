...
const MemberListExist = ({
    ...
    confirm({
      ...
      content: (
        <span>
          ...
          muốn xoá <b style={{ color: "red" }}>{x_strMemberFullName}</b> khỏi hệ
          thống?
        </span>
      ),
      okText: "Đồng Ý",
  ...
  const DeleteMemberById = async (x_strMemberId, x_strMemberFullName) => {
    ...
    const strApiURL = `${API_URL}api/Member/DeleteMember`;
    await axios
      .delete(strApiURL, {
        withCredentials: true,
        params: param,
      ...
      .then((response) => {
        if (response.data.isSuccess) {
          removeMember(x_strMemberId);
          var strMess = "Xoá thành viên " + x_strMemberFullName + " thành công";
  ...
  const removeMember = (x_strMemberId) => {
    if (m_lstMemberList && m_lstMemberList.length > 0) {
      const index = m_lstMemberList.findIndex(
        (member) => member.memberId === x_strMemberId
      );

      if (index !== -1) {
        const updatedList = [...m_lstMemberList];
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 50,
      render: (joinDate) => (
        <span>{moment(joinDate).format("DD/MM/yyyy")}</span>
      ),
    },
    {
      title: "Avatar",
      ...
      render: (avatarPath) => (
        <Image
          src={GetFullPath(avatarPath, viewtoken)}
    ...
    },
    {
      ...
      width: 90,
      render: (score) => (
        <Tag color={score > 0 ? "success" : "error"}>{score.toFixed(2)}</Tag>
      ),
    },
    {
      title: "Tổng Điểm",
      ...
      render: (text, record) => (
        <Row>
          {permission.permissions.includes(2001) ? (
            <NavLink
              to={
                "/dashboard/member-list/member-info?memberid=" + record.memberId
              }
            >
              <Col style={{ padding: 2 }}>
                <Button
          ...
          <NavLink
            to={
              "/dashboard/member-list/activity-history-member?memberid=" +
              record.memberId
            }
          >
            <Col style={{ padding: 2 }}>
          ...
          {permission.permissions.includes(2002) ? (
            <Col style={{ padding: 2 }}>
              <Button
                disabled={m_bLoading}
                onClick={() => {
        ...
        <Col span={24} xs={24} xl={24}>
          <Card style={{ marginTop: 10, marginBottom: 10 }}>
            <Row>
              <Col span={24} xs={24} xl={24}>
                <Search
                  placeholder="Nhập thông tin cần tìm kiếm (VD: Tên/MSSV/SĐT)"
                  enterButton
                  disabled={m_bLoading}
                  onSearch={(value) => {
                ...
                <Col span={24} xs={24} xl={24} style={{ marginTop: 10 }}>
                  <Row justify="start">
                    ...
                    <Col span={6} xs={24} xl={6}>
                      ...
                      >
                        Tải Xuống Danh Sách Thành Viên
                      </Button>
                    </Col>
                  </Row>
                </Col>
...
export default connect(mapStateToProps, mapDispatchToProps)(MemberListExist);
